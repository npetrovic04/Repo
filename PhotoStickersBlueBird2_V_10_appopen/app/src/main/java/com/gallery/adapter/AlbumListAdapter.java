...
public class AlbumListAdapter extends BaseAdapter
{
    ...
    public View getView(int position, View view, ViewGroup viewGroup)
    {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N)
            holder.photo_album_element_count.setText(albumInfo.getPhotoUris().size() + "");
        else
        ...
        else
        {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N)
                Glide.with(context).load(albumInfo.getCoverUri()).into(holder.photo_album_cover);
            else
                Glide.with(context).load(albumInfo.getCoverPath()).into(holder.photo_album_cover);
        }
